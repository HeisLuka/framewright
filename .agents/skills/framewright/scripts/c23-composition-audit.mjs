...
const adaptive=manifest.items.filter(x=>x.mode==='adaptive'),results=[];for(let i=0;i<adaptive.length;i++){const r=await inspect(adaptive[i]);results.push(r);console.log(`${i+1}/${adaptive.length} ${r.bookId}: side=${r.composition?.textSide} focus=${r.composition?.focusX},${r.composition?.focusY} conf=${r.composition?.confidence}`);}
const failures=results.filter(x=>x.composition?.source!=='cover');const left=results.filter(x=>x.composition?.textSide==='left').length,right=results.filter(x=>x.composition?.textSide==='right').length;const confidences=results.map(x=>x.composition?.confidence||0);const report={schema:'framewright-c23-composition-audit-v1',count:results.length,failures:failures.map(x=>x.bookId),sides:{left,right},confidence:{min:Math.min(...confidences),max:Math.max(...confidences),mean:+(confidences.reduce((a,b)=>a+b,0)/confidences.length).toFixed(4)},results};
fs.mkdirSync(path.dirname(reportPath),{recursive:true});fs.writeFileSync(reportPath,JSON.stringify(report,null,2));if(report.count!==36)throw new Error(`expected 36 adaptive covers, got ${report.count}`);if(failures.length)throw new Error(`analysis failures: ${failures.length}`);if(left<4)throw new Error(`placement policy collapsed: only ${left} left-text decisions`);if(right<4)throw new Error(`placement policy collapsed: only ${right} right-text decisions`);console.log(JSON.stringify(report,null,2));
