...
const report={schema:'framewright-c25-typography-audit-v2',times:TIMES,books:rows,meanAcrossBooks:+(rows.reduce((a,x)=>a+x.meanPair,0)/rows.length).toFixed(6),minAcrossBooks:+Math.min(...rows.map(x=>x.minPair)).toFixed(6)};
...
console.log(JSON.stringify({books:rows.length,mean:report.meanAcrossBooks,min:report.minAcrossBooks},null,2));
