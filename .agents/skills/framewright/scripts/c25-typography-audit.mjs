...
for(const [file,family] of [
 ['/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf','DejaVu Sans'],['/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf','DejaVu Sans'],
 ['/usr/share/fonts/truetype/dejavu/DejaVuSansCondensed.ttf','DejaVu Sans Condensed'],['/usr/share/fonts/truetype/dejavu/DejaVuSansCondensed-Bold.ttf','DejaVu Sans Condensed'],
 ['/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf','DejaVu Serif'],['/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf','DejaVu Serif']
...
function compactEvents(events){return events.map(e=>({kind:e.kind,role:e.role,family:e.family,sourceSize:e.sourceSize,requestedSize:e.requestedSize,finalSize:e.finalSize,weight:e.weight,lines:e.lines,maxW:e.maxW,lineHeight:e.lineHeight,tracking:e.tracking??null,box:e.box}));}
function fitSignature(x){return JSON.stringify(x.samples.map(s=>s.events.map(e=>[e.kind,e.role,e.family,e.finalSize,e.weight,e.lines,e.maxW,e.tracking])));}
...
for(const bookId of books){
 ...
 const telemetry=Object.fromEntries(b.map(x=>[x.entry.typography,{policy:x.policy,samples:x.samples.map((s,k)=>({time:TIMES[k],events:s.events}))}]));
...
if(report.roiMeanAcrossBooks<.12)throw new Error(`typography ROI diversity too low: mean ${report.roiMeanAcrossBooks} < .12`);
if(report.roiMinAcrossBooks<.06)throw new Error(`typography ROI pair collapsed: min ${report.roiMinAcrossBooks} < .06`);
