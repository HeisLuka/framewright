...
for(const bookId of books){
 ...
 rows.push({bookId,style:b[0].entry.style,language:b[0].entry.language,minPair:+Math.min(...pairs.map(x=>x.mean)).toFixed(6),meanPair:+(pairs.reduce((a,x)=>a+x.mean,0)/pairs.length).toFixed(6),minRoiPair:+Math.min(...pairs.map(x=>x.roiMean)).toFixed(6),meanRoiPair:+(pairs.reduce((a,x)=>a+x.roiMean,0)/pairs.length).toFixed(6),pairs,telemetry});
}
const report={schema:'framewright-c25-typography-audit-v3',times:TIMES,books:rows,meanAcrossBooks:+(rows.reduce((a,x)=>a+x.meanPair,0)/rows.length).toFixed(6),minAcrossBooks:+Math.min(...rows.map(x=>x.minPair)).toFixed(6),roiMeanAcrossBooks:+(rows.reduce((a,x)=>a+x.meanRoiPair,0)/rows.length).toFixed(6),roiMinAcrossBooks:+Math.min(...rows.map(x=>x.minRoiPair)).toFixed(6)};
fs.mkdirSync(path.dirname(outPath),{recursive:true});fs.writeFileSync(outPath,JSON.stringify(report,null,2));
console.log(JSON.stringify({books:rows.length,mean:report.meanAcrossBooks,min:report.minAcrossBooks,roiMean:report.roiMeanAcrossBooks,roiMin:report.roiMinAcrossBooks},null,2));
if(report.meanAcrossBooks<.028)throw new Error(`typography systems too similar: mean ${report.meanAcrossBooks} < .028`);
if(report.minAcrossBooks<.010)throw new Error(`typography pair collapsed: min ${report.minAcrossBooks} < .010`);
