...
const itemById=new Map(manifest.items.map(x=>[x.id,x])),results=[];let warnings=0;
for(const row of batch.results){const item=itemById.get(row.id);if(!item)throw new Error(`batch row missing manifest item ${row.id}`);warnings+=row.layoutWarnings.length;results.push({...item,output:row.output,outputSha256:sha(row.output),totalMs:row.totalMs,outputBytes:row.outputBytes,layoutWarnings:row.layoutWarnings.length});}
...
const summary={schema:'framewright-c27-narrative-render-summary-v1',renders:batch.count,books:books.length,layoutWarnings:warnings,semanticAuditOk:semantic.ok===true,semanticPlans:semantic.plans,deterministicReplays:semantic.deterministic_replays,copyAtomsVerified:semantic.copy_atoms_verified,renderAuditErrors:renderAudit.errors?.length||0,renderAuditCount:renderAudit.count,bookGroupsWithSixDistinctOutputs:uniqueGroups,revealOrderGroups:orderedRevealGroups,bookEvidence,perVideoMs:stats(results.map(x=>x.totalMs)),outputBytes:stats(results.map(x=>x.outputBytes)),videosPerHour:batch.videosPerHour,peakCombinedRssBytes:batch.peakCombinedRssBytes};
fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(summary,null,2));
if(batch.count!==36||results.length!==36)throw new Error(`expected 36 renders, got ${results.length}`);if(books.length!==6)throw new Error(`expected 6 books, got ${books.length}`);if(warnings!==0)throw new Error(`layout warnings ${warnings}`);if(!summary.semanticAuditOk||semantic.plans!==36||semantic.deterministic_replays!==36)throw new Error('semantic audit incomplete');if(summary.renderAuditErrors!==0||summary.renderAuditCount!==36)throw new Error('render audit incomplete');if(uniqueGroups!==6)throw new Error(`distinct-output coverage ${uniqueGroups}/6`);if(orderedRevealGroups!==12)throw new Error(`reveal ordering ${orderedRevealGroups}/12`);console.log(JSON.stringify(summary,null,2));
