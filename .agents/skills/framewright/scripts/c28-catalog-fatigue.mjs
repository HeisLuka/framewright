...
function bookConstraints(bookIndex){
  return{
    duration_s:pick(durations,bookIndex),
    platform_profile:pick(platforms,bookIndex),
    palette_family:`cover-${bookIndex%12}`,
    composition_policy:'c23-cover-derived'
  };
}
function makeCandidate(bookIndex,k){
  const fixed=bookConstraints(bookIndex),base=hash(`book-${bookIndex}`),book_id=`book-${String(bookIndex+1).padStart(2,'0')}`;
  // Candidate 0 models a valid but repetitive default selection. Book-derived constraints stay fixed across every candidate.
  if(k===0)return{book_id,candidate:k,visual_system:pick(systems,bookIndex),creative_variant:'hook-first',opening_grammar:'hook-led',typography_system:'baseline',...fixed};
  const n=(base+k*2654435761)>>>0;
  return{book_id,candidate:k,visual_system:pick(systems,n),creative_variant:pick(variants,n>>>3),opening_grammar:pick(openings,n>>>6),typography_system:pick(typography,n>>>10),...fixed};
}
...
function overlap(a,b){
  ...
  s+=a.typography_system===b.typography_system?1.2:0;
  // Duration is book-derived and not rerankable, but it still affects perceived catalog rhythm.
  s+=a.duration_s===b.duration_s?0.8:0;
  return s;
}
function recentPenalty(c,selected){
  const recent=selected.slice(-recentWindow);let p=0;
  for(let i=0;i<recent.length;i++)p+=overlap(c,recent[i])*(1+(i+1)/recent.length*.35);
  const last=recent.at(-1);
  if(last){if(c.visual_system===last.visual_system)p+=2.4;if(c.opening_grammar===last.opening_grammar)p+=2.8;if(c.creative_variant===last.creative_variant)p+=1.6;if(c.typography_system===last.typography_system)p+=1.0;}
  return p;
}
function rerank(groups){
  const selected=[],decisions=[];
  for(const group of groups){
    // All candidates in a group must preserve the same non-rerankable book constraints.
    const fixed=['duration_s','platform_profile','palette_family','composition_policy'];
    for(const f of fixed)if(group.some(c=>c[f]!==group[0][f]))throw new Error(`${group[0].book_id}: candidate changed fixed constraint ${f}`);
...
const naive=groups.map(g=>g[0]),aware=rerank(groups);
