...
import {
  C33_POLICY_SCHEMA,
  C33_PLAN_SCHEMA,
  C33_ASSIGNMENT_SCHEMA,
  buildTrafficPlan,
  assignTraffic
} from './c33-exploration-holdout-selector.mjs';

const outDir=path.resolve(process.argv[2]||'artifacts/c33');
...
const portfolio={
  portfolio_id:'portfolio-c33-audit',
...
const learningPolicy={
  ...
  policy_version:'c30-learning-c33-audit-v1',
...
const selectorPolicy={
  ...
  policy_version:'c33-selector-v1',
  experiment_id:'exp-c33-audit',
  primary_metric:'downstream_open',
  assignment_seed:'selector-seed-3301',
...
if(planA.schema!==C33_PLAN_SCHEMA)throw new Error('wrong C33 plan schema');
...
for(let i=0;i<subjectCount;i++){
  ...
  if(a.schema!==C33_ASSIGNMENT_SCHEMA||b.schema!==C33_ASSIGNMENT_SCHEMA||c.schema!==C33_ASSIGNMENT_SCHEMA)throw new Error('wrong assignment schema');
...
if(!Object.values(gates).every(Boolean))throw new Error(`C33 audit gate failed: ${JSON.stringify(gates)}`);
const report={
  schema:'framewright-c33-exploration-holdout-selector-audit-v1',
...
const summary=`# C33 exploration/holdout selector audit\n\n`+
