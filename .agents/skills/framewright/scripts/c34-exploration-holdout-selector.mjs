...
export const C34_POLICY_SCHEMA='framewright-c34-selector-policy-v1';
export const C34_PLAN_SCHEMA='framewright-c34-traffic-plan-v1';
export const C34_ASSIGNMENT_SCHEMA='framewright-c34-assignment-receipt-v1';
...
function armId(experimentId,policyVersion,lane,creativeId){return `c34a1_${sha256(`${experimentId}\u001f${policyVersion}\u001f${lane}\u001f${creativeId}`)}`;}

export function validateSelectorPolicy(policy){
  if(!isObject(policy))throw new Error('selector policy must be an object');
  if(policy.schema!==C34_POLICY_SCHEMA)throw new Error(`wrong selector policy schema: ${policy.schema}`);
  if(policy.metric_weights!==undefined||policy.objective_weights!==undefined||policy.composite_score!==undefined)throw new Error('composite metric weights are forbidden in C34; declare exactly one primary_metric');
  return {
...
export function buildTrafficPlan({snapshot,policy}){
  ...
  return {...body,traffic_plan_id:`c34t1_${sha256(stableStringify(body))}`};
}
...
export function assignTraffic({plan,subject_key}){
  if(!isObject(plan)||plan.schema!==C34_PLAN_SCHEMA)throw new Error('invalid C34 traffic plan');
