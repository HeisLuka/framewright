...
async function boot(payload,seed){
  ...
  const window={FRAMEWRIGHT_PAYLOAD:payload},location={search:`?f=0&w=1080&s=${seed}&profile=vertical`};const sandbox={window,document,Image,URLSearchParams,location,console,performance,setTimeout,clearTimeout,requestAnimationFrame(){return 0;}};sandbox.globalThis=sandbox;window.window=window;window.document=document;window.location=location;
  vm.runInContext(source,vm.createContext(sandbox),{filename:htmlPath});const deadline=performance.now()+10000;while(!window.__ready&&performance.now()<deadline)await new Promise(r=>setTimeout(r,10));if(!window.__ready)throw new Error('boot timeout');if(window.__bootError)throw new Error(window.__bootError);return{window,canvas};
}
...
for(const style of styles){
  for(const platform of platforms){
    const payload={...input.book,cover_url:`../book-ad-v0/generated-e08/${coverBase}`,visual_system:style,creative_variant:'hook-first',delivery_profile:'vertical',art_direction_mode:'cover',cover_composition_mode:'adaptive',opening_grammar:'hook-led',motion_density:'choreography-v2',typography_system:'baseline',pacing_mode:'c27-narrative-v1',platform_profile:platform,narrative_plan:plan};
    const {window}=await boot(payload,plan.seed),contract=window.__C27_RENDER_CONTRACT?.(),actualPlan=window.__C27_NARRATIVE_PLAN?.(),ui=window.__C26_PLATFORM_UI?.();
    if(!contract||contract.planId!==plan.narrative_plan_id||contract.totalFrames!==plan.total_frames)errors.push(`${style}/${platform}: render contract drift`);
    if(stableJson(actualPlan)!==stableJson(plan))errors.push(`${style}/${platform}: NarrativePlan mutated in renderer`);
    if(window.RISO.total!==plan.total_frames)errors.push(`${style}/${platform}: total ${window.RISO.total} != ${plan.total_frames}`);
    const actualPlates=window.RISO.plates.map(x=>({role:x.name,frames:x.len}));if(stableJson(actualPlates)!==stableJson(expectedPlates))errors.push(`${style}/${platform}: plate schedule drift`);
    if(window.RISO.payload.visual_system!==style)errors.push(`${style}/${platform}: style drift ${window.RISO.payload.visual_system}`);
    if(window.RISO.payload.platform_profile!==platform)errors.push(`${style}/${platform}: platform drift ${window.RISO.payload.platform_profile}`);
    if(!ui?.safeRect)errors.push(`${style}/${platform}: C26 safe profile missing`);
    let cursor=0,sceneEvents=0,sceneViolations=0;
    for(const role of plan.roles){
      for(const frac of fractions){
        const f=cursor+Math.min(role.frames-1,Math.max(0,Math.round((role.frames-1)*frac)));
        if(window.__C26_BEGIN_CAPTURE)window.__C26_BEGIN_CAPTURE();window.RISO.frame(f,renderWidth,plan.seed);const cap=window.__C26_END_CAPTURE?window.__C26_END_CAPTURE():{events:[]};sampledFrames++;
        for(const e of cap.events||[]){sceneEvents++;totalEvents++;if(ui?.safeRect&&!inside(e.box,ui.safeRect)){sceneViolations++;safeViolations++;errors.push(`${style}/${platform}/${role.role}@${f}: ${e.kind||'event'} outside safeRect ${JSON.stringify(e.box)}`);}}
      }
      cursor+=role.frames;
    }
    rows.push({style,platform,narrativePlanId:plan.narrative_plan_id,roles:actualPlates,totalFrames:window.RISO.total,safeRect:ui?.safeRect||null,sampledFrames:plan.roles.length*fractions.length,eventCount:sceneEvents,violationCount:sceneViolations});
    console.log(`${style}/${platform}: roles=${actualPlates.map(x=>`${x.role}:${x.frames}`).join('>')} events=${sceneEvents} violations=${sceneViolations}`);
  }
}
