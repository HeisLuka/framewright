...
function narrativePlanFixture(raw,copyId){
  ...
  const plan={
    ...
    policy_version:'c27-narrative-v1',
    book_id:raw.book_id,
    duration_seconds,
    fps:30,
    total_frames:duration_seconds*30,
    reveal_timing:'mid',
    cta_treatment:copyId==='short'?'soft_reveal':'direct',
    roles,
  };
...
function compileCreativeRealization(raw,style,narrativePlan){
  const content=canonicalContent(raw);
  const plan={
    schema:'framewright-creative-realization-plan-v0',
    payload_sha256:sha(content),
    book_id:content.book_id,
    narrative_plan_id:narrativePlan.narrative_plan_id,
    duration_ms:narrativePlan.duration_seconds*1000,
    total_frames:narrativePlan.total_frames,
    mechanisms:creativeMechanisms(style),
  };
  plan.creative_realization_id=id('fwcr0',plan);
  return{content,plan};
}
...
function compileScene(creative,platform,runtime=RUNTIME){
  const delivery=deliveryProfile(platform);
  const scene={schema:'framewright-compiled-scene-v0',creative_realization_id:creative.creative_realization_id,duration_ms:creative.duration_ms,total_frames:creative.total_frames,delivery,runtime};
...
for(const raw of wrappers){
  const semanticKey=`${raw.book_id}|${raw.visual_system}`;
  let creative=bySemanticKey.get(semanticKey);
  if(!creative){
    const narrative=narrativePlanFixture(raw,raw.copyId);narrativeRows.push(narrative);
    creative=compileCreativeRealization(raw,raw.visual_system,narrative).plan;
    bySemanticKey.set(semanticKey,creative);creativeRows.push(creative);
  }else{
    const narrative=narrativePlanFixture(raw,raw.copyId);
    const again=compileCreativeRealization(raw,raw.visual_system,narrative).plan;
    assert(again.creative_realization_id===creative.creative_realization_id,`${semanticKey}: platform leaked into creative identity`);
  }
  const scene=compileScene(creative,raw.platform_profile);sceneRows.push({...scene,platform:raw.platform_profile,audio:audioPrecondition(scene)});
}
