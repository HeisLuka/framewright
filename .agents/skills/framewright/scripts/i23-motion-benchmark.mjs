...
const top1=rows.filter(r=>r.top1).length,top3=rows.filter(r=>r.top3).length,accepted=rows.filter(r=>r.state==='accepted').length,correct=rows.filter(r=>r.correct).length,mrr=rows.reduce((sum,r)=>sum+(r.motion_rank?1/r.motion_rank:0),0)/rows.length;
const summary={schema:'i23-physical-motion-benchmark-v1',version:1,videos:rows.length,hard_gates:{source_hash_exact:rows.filter(r=>r.source_hash_exact).length,tracks_observed:rows.filter(r=>r.track_sample_count>=5).length,zero_forward_leakage:rows.reduce((n,r)=>n+r.leakage_violations.length,0)===0},diagnostic:{accepted,correct,top1,top3,top1_rate:Number((top1/rows.length).toFixed(6)),top3_rate:Number((top3/rows.length).toFixed(6)),mean_reciprocal_rank:Number(mrr.toFixed(6)),mean_motion_coverage:Number((rows.reduce((s,r)=>s+r.motion_coverage,0)/rows.length).toFixed(6))},rows};
