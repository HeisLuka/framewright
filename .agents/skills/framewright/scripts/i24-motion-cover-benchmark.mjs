...
function group(rows,key){
  ...
  for(const row of rows){
    ...
    if(!values[value])values[value]={videos:0,phase_a_ok:0,c51_accepted:0,top1:0,top3:0,tracks_observed:0};
...
for(const artifact of [...canonical.artifacts].sort((a,b)=>a.render_spec_id.localeCompare(b.render_spec_id))){
  ...
  if(!phase.ok){rows.push({selection_id:selection,render_spec_id:id,book_id:bookId,ground_truth_motion_grammar:truth,phase_a_ok:false,phase_a_stage:phase.stage,phase_a_error:phase.error,source_hash_exact:false,state:null,reason:null,inferred_motion_grammar:null,fit_residual:null,runner_up_margin:null,latent_direction_sign:null,motion_rank:null,top1:false,top3:false,track_sample_count:0,target_sample_count:0,detected_sample_count:0,motion_coverage:0,candidates:[],correct:false,leakage_violations:[]});continue;}
...
const summary={
  schema:'i24-physical-motion-cover-holdout-v3',version:3,videos:n,
  hard_gates:{phase_a_completed:phaseAOk,source_hash_exact:sourceExact,tracks_observed:tracks,zero_forward_leakage:leakage===0,c51_accepted:c51Accepted,c51_correct_top1:rows.filter(r=>r.correct&&r.top1).length},
  diagnostic:{accepted:c51Accepted,correct:c51Correct,top1,top3,top1_rate:Number((top1/n).toFixed(6)),top3_rate:Number((top3/n).toFixed(6)),mean_reciprocal_rank:Number(mrr.toFixed(6)),mean_motion_coverage:Number((rows.reduce((s,r)=>s+r.motion_coverage,0)/n).toFixed(6)),by_book:group(rows,'book_id'),by_motion_family:group(rows,'ground_truth_motion_grammar')},
  rows,
};
...
if(summary.hard_gates.c51_accepted!==n)throw new Error(`C51 acceptance gate failed ${summary.hard_gates.c51_accepted}/${n}`);
if(summary.hard_gates.c51_correct_top1!==n)throw new Error(`C40 hidden top-1 gate failed ${summary.hard_gates.c51_correct_top1}/${n}`);
