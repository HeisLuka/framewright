...
#   scripts/init.sh [target_dir=.] [--force]
# Creates: index.html (from the skeleton), scripts/, audio.mjs (template), storyboard.md, package.json, .gitignore
set -euo pipefail
...
for f in look.mjs render.mjs build.sh make.sh inject.mjs portrait.sh trace.py doctor.sh; do put "$HERE/$f" "scripts/$f"; done
