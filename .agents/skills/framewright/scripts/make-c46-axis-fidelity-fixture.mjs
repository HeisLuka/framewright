...
const baseline={
  ...
  asset_staging:'stage_hero_cover_v2',
...
function valueFor(selection,axis){return axis==='graphic_devices'?selection.graphic_devices[0]:selection[axis];}
const cases=[{axis_under_test:'baseline',option_id:'baseline',selection:structuredClone(baseline)}];
for(const axis of ['structural_layout','visual_system','typography','motion_grammar','asset_staging','graphic_devices']){
  for(const option of axisOptions[axis]){
    if(option===valueFor(baseline,axis))continue;
    const selection=structuredClone(baseline);
    if(axis==='graphic_devices')selection.graphic_devices=[option];else selection[axis]=option;
    cases.push({axis_under_test:axis,option_id:option,selection});
  }
}
if(cases.length!==31)throw new Error(`expected 31 OFAT cases, got ${cases.length}`);
...
for(const [index,testCase] of cases.entries()){
  const receipt=composeExplicitTemplateVariant({selection:testCase.selection,context,registry});
  ...
  const selectionId=`c46-${String(index+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-c46/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,c46_control:{axis_under_test:testCase.axis_under_test,option_id:testCase.option_id}};
