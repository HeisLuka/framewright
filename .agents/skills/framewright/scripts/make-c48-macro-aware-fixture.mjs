...
} from '../../../../contracts/batch-template-diversity-v2.mjs';

const fixtureDir=path.resolve(process.argv[2]||'.bench/c48/c27-fixtures');
const html=path.resolve(process.argv[3]||'examples/book-ad-systems/index-c45-template-variant.html');
const audioSource=path.resolve(process.argv[4]||'.bench/c48/source.wav');
const audioArtifact=path.resolve(process.argv[5]||'.bench/c48/canonical-audio.m4a');
const outDir=path.resolve(process.argv[6]||'.bench/c48/input');
const BATCH_SIZE=40;
...
for(const file of [html,audioSource,audioArtifact,path.join(fixtureDir,'manifest.json')])if(!fs.existsSync(file))throw new Error(`missing C48 input ${file}`);
...
if(plan.duration_seconds!==9)throw new Error(`C48 corpus is frozen at canonical 9s C27, got ${plan.duration_seconds}s`);
...
const runtimeAssetDir=path.join(path.dirname(html),'generated-c48');fs.mkdirSync(runtimeAssetDir,{recursive:true});
...
const diversityPolicyV1=buildDefaultBatchTemplateDiversityPolicy();
const diversityPolicyV2=buildDefaultBatchTemplateDiversityPolicyV2();
const byVariant=new Map();
for(let i=0;i<3000;i+=1){
  const receipt=composeAutomaticTemplateVariant({subject_key:`c48-${book.book_id}-${String(i).padStart(4,'0')}`,seed:48,context,policy:autoPolicy,registry});
...
if(candidates.length<1000)throw new Error(`expected >=1000 unique candidates, got ${candidates.length}`);
const selectionSeed=4801;
const v1Control=selectDiverseTemplateBatch({candidates,batch_size:BATCH_SIZE,seed:selectionSeed,policy:diversityPolicyV1});
const batch=selectDiverseTemplateBatchV2({candidates,batch_size:BATCH_SIZE,seed:selectionSeed,policy:diversityPolicyV2});
...
const artifactBytes=fs.readFileSync(audioArtifact),canonicalAudio={schema:'framewright-canonical-audio-v1',audio_spec_id:audioSpec.audio_spec_id,sha256:shaBytes(artifactBytes),bytes:artifactBytes.byteLength,media_type:'audio/mp4',storage_uri:`artifact://c48/${path.basename(audioArtifact)}`,provenance:{encoder:'ffmpeg-github-runner',policy:audioSpec.codec.policy}};
...
for(const assignment of batch.assignments){
  ...
  const selectionId=`c48-${String(assignment.index+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-c48/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,c48_batch:{batch_id:batch.batch_id,index:assignment.index}};
  ...
  const template={id:'book-ad-template-variant',version:'c48-macro-aware-v1',sha256:templateSha256};
  selected.push({selection_id:selectionId,creative:{schema:'newboo-creative-spec-v1',book_id:book.book_id,payload_sha256:payloadSha256,template,visual_system:{id:variant.axes.visual_system,version:'v2'},structural_variant:variant.axes.structural_layout,hook:{source:'c27_narrative_plan',text:hook,source_ref:`${plan.narrative_plan_id}:hook`},motion:{profile:variant.axes.motion_grammar,version:'v2'},art_direction:{mode:'template-variant',algorithm:'c47-macro-aware-batch-v2',source_cover_sha256:coverSha256,palette:{background:book.background||'#eee8dc',surface:'#ffffff',ink:book.ink||'#111111',accent:book.accent||'#cc332d',secondary:'#315A7D'}},seed:plan.seed,assets:[{role:'cover',sha256:coverSha256,media_type:'image/png',uri:`asset://generated-c48/${coverName}`}],template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id},timeline:{source:`c27:${plan.narrative_plan_id}`,policy_version:plan.policy_version,duration_ms:durationMs,frame_count:frameCount},requested_delivery_profile_ids:['vertical-c48-v1'],render_assets:[{role:'scene_payload',sha256:payloadSha256,media_type:'application/json',uri:`execution://${selectionId}/payload`}],audio:{spec:audioSpec,artifact:canonicalAudio},execution:{html:rel(html),payload:rel(payloadFile),audio:rel(audioArtifact),template_id:template.id,template_version:template.version}});
  variantRows.push({index:assignment.index,selection_id:selectionId,candidate_id:assignment.candidate_id,template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id,payload_sha256:payloadSha256,axes:variant.axes,semantic_schedule_sha256:sceneProgram.semantic_schedule_sha256,distance_from_previous:assignment.distance_from_previous,macro_distance_from_previous:assignment.macro_distance_from_previous});
}
const request={schema:'framewright-c19-campaign-request-v1',campaign_id:'c48-macro-aware-template-diversity-40',compiler_policy_version:'c19-delivery-package-v1',runtime:{class:'FAST',scene_contract_version:'canvas-scene-v1',environment_id:'video-worker-chromium-v1',renderer:{id:'webcodecs-h264',version:'r35-standard-v1'},encoder:{video_codec:'avc1.420028',bitrate_bps:3000000,pixel_format:'yuv420p'},muxer:{id:'ffmpeg-stream-copy-aac',version:'6.1.1'}},delivery_profiles:[{id:'vertical-c48-v1',width:1080,height:1920,fps:30,safe_area_profile:'c26-ui-safe-v1-2026-09-17:youtube_shorts',platform_ui_profile:'youtube_shorts',platform_ui_version:'c26-ui-safe-v1-2026-09-17'}],selected,reserves:[{reserve_id:'c48-unrendered-control',reason:'prove-reserve-suppression'}]};
const campaignFile=path.join(outDir,'campaign.json'),batchFile=path.join(outDir,'batch-receipt-v2.json'),controlFile=path.join(outDir,'batch-receipt-v1-control.json'),variantFile=path.join(outDir,'variant-manifest.json');
fs.writeFileSync(campaignFile,JSON.stringify(request,null,2)+'\n');
fs.writeFileSync(batchFile,JSON.stringify(batch,null,2)+'\n');
fs.writeFileSync(controlFile,JSON.stringify(v1Control,null,2)+'\n');
fs.writeFileSync(variantFile,JSON.stringify({schema:'c48-variant-manifest-v1',registry_id:registry.registry_id,auto_policy_id:autoPolicy.policy_id,diversity_policy_v1_id:diversityPolicyV1.policy_id,diversity_policy_v2_id:diversityPolicyV2.policy_id,narrative_plan_id:plan.narrative_plan_id,book_id:book.book_id,semantic_projection:narrative,cover_sha256:coverSha256,template_sha256:templateSha256,candidate_count:candidates.length,count:variantRows.length,variants:variantRows},null,2)+'\n');
console.log(JSON.stringify({campaign:campaignFile,batchReceiptV2:batchFile,v1ControlReceipt:controlFile,variantManifest:variantFile,candidateCount:candidates.length,count:variantRows.length,narrativePlanId:plan.narrative_plan_id,semanticScheduleSha256:variantRows[0]?.semantic_schedule_sha256,batchId:batch.batch_id,v1ControlBatchId:v1Control.batch_id,macroAxes:batch.macro_axes,minLookbackDistance:batch.min_observed_lookback_distance,minMacroLookbackDistance:batch.min_observed_macro_lookback_distance,uniqueVariants:new Set(variantRows.map(r=>r.template_variant_id)).size,uniqueScenePrograms:new Set(variantRows.map(r=>r.scene_program_id)).size,templateSha256,coverSha256,audioSpecId:audioSpec.audio_spec_id},null,2));
