...
} from '../../../../contracts/template-campaign-selection-v1.mjs';
import {
...
const fixtureDir=path.resolve(process.argv[2]||'.bench/c49/c27-fixtures');
const html=path.resolve(process.argv[3]||'examples/book-ad-systems/index-c45-template-variant.html');
const audioSource=path.resolve(process.argv[4]||'.bench/c49/source.wav');
const audioArtifact=path.resolve(process.argv[5]||'.bench/c49/canonical-audio.m4a');
const outDir=path.resolve(process.argv[6]||'.bench/c49/input');
...
for(const file of [html,audioSource,audioArtifact,path.join(fixtureDir,'manifest.json')])if(!fs.existsSync(file))throw new Error(`missing C49 input ${file}`);
...
const book=input.book;if(plan.duration_seconds!==9)throw new Error(`C49 physical fixture expects canonical 9s C27, got ${plan.duration_seconds}s`);
...
const runtimeAssetDir=path.join(path.dirname(html),'generated-c49');fs.mkdirSync(runtimeAssetDir,{recursive:true});
...
for(let i=0;i<1800;i+=1){
  const receipt=composeAutomaticTemplateVariant({subject_key:`c49-production-${book.book_id}-${String(i).padStart(4,'0')}`,seed:49,context,policy:autoPolicy,registry});
...
const candidates=[...byVariant.values()];if(candidates.length<1000)throw new Error(`expected >=1000 semantic candidates, got ${candidates.length}`);
const selection=selectCurrentTemplateCampaignBatch({candidates,batch_size:6,selection_seed:4901});
...
const artifactBytes=fs.readFileSync(audioArtifact),canonicalAudio={schema:'framewright-canonical-audio-v1',audio_spec_id:audioSpec.audio_spec_id,sha256:shaBytes(artifactBytes),bytes:artifactBytes.byteLength,media_type:'audio/mp4',storage_uri:`artifact://c49/${path.basename(audioArtifact)}`,provenance:{encoder:'ffmpeg-github-runner',policy:audioSpec.codec.policy}};assertCanonicalAudioArtifact(audioSpec,canonicalAudio,artifactBytes);
...
for(const [index,candidateId] of selection.ordered_selected_candidate_ids.entries()){
  ...
  const selectionId=`c49-${String(index+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-c49/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,c49_selection:{selection_id:selection.selection_id,candidate_id:candidateId,index}};
  ...
  const hook=narrative.roles.find(role=>role.role==='hook')?.text||'';
  const template={id:'book-ad-template-variant',version:'c49-production-selection-v1',sha256:templateSha256};
  const row={selection_id:selectionId,creative:{schema:'newboo-creative-spec-v1',book_id:book.book_id,payload_sha256:payloadSha256,template,visual_system:{id:variant.axes.visual_system,version:'v2'},structural_variant:variant.axes.structural_layout,hook:{source:'c27_narrative_plan',text:hook,source_ref:`${plan.narrative_plan_id}:hook`},motion:{profile:variant.axes.motion_grammar,version:'v2'},art_direction:{mode:'template-variant',algorithm:'c49-production-selection-v1',source_cover_sha256:coverSha256,palette:{background:book.background||'#eee8dc',surface:'#ffffff',ink:book.ink||'#111111',accent:book.accent||'#cc332d',secondary:'#315A7D'}},seed:plan.seed,assets:[{role:'cover',sha256:coverSha256,media_type:'image/png',uri:`asset://generated-c49/${coverName}`}],template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id},timeline:{source:`c27:${plan.narrative_plan_id}`,policy_version:plan.policy_version,duration_ms:durationMs,frame_count:frameCount},requested_delivery_profile_ids:['vertical-c49-v1'],render_assets:[{role:'scene_payload',sha256:payloadSha256,media_type:'application/json',uri:`execution://${selectionId}/payload`}],audio:{spec:audioSpec,artifact:canonicalAudio},execution:{html:rel(html),payload:rel(payloadFile),audio:rel(audioArtifact),template_id:template.id,template_version:template.version}};
  materialized.push({candidate_id:candidateId,row});rows.push({index,candidate_id:candidateId,selection_id:selectionId,template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id,payload_sha256:payloadSha256,axes:variant.axes});
}
...
const deliveryProfiles=[{id:'vertical-c49-v1',width:1080,height:1920,fps:30,safe_area_profile:'c26-ui-safe-v1-2026-09-17:youtube_shorts',platform_ui_profile:'youtube_shorts',platform_ui_version:'c26-ui-safe-v1-2026-09-17'}];
const request=assembleC19TemplateCampaignRequest({selection,materialized_selected:materialized,campaign_id:'c49-production-template-selection',runtime,delivery_profiles:deliveryProfiles});
...
fs.writeFileSync(path.join(outDir,'materialization-manifest.json'),JSON.stringify({schema:'c49-production-materialization-manifest-v1',candidate_count:candidates.length,selected_count:rows.length,reserve_count:request.reserves.length,selection_id:selection.selection_id,policy_id:selection.provenance.policy_id,batch_id:selection.provenance.batch_id,narrative_plan_id:plan.narrative_plan_id,template_sha256:templateSha256,cover_sha256:coverSha256,rows},null,2)+'\n');
console.log(JSON.stringify({schema:'c49-production-selection-fixture-v1',candidateCount:candidates.length,selectedCount:rows.length,reserveCount:request.reserves.length,physicalPayloadFiles:fs.readdirSync(outDir).filter(name=>name.endsWith('.payload.json')).length,selectionId:selection.selection_id,policyId:selection.provenance.policy_id,batchId:selection.provenance.batch_id,macroAxes:selection.provenance.receipt.macro_axes,narrativePlanId:plan.narrative_plan_id,templateSha256,coverSha256,audioSpecId:audioSpec.audio_spec_id},null,2));
