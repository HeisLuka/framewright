...
const candidates=motionFamilies.map(motion_grammar=>{
  ...
  return{candidate_id:`candidate_${motion_grammar}`,template_variant:receipt.template_variant};
});
...
// Deliberately do not invoke the C49 production diversity selector here. C49 correctly
// requires macro-axis distance and share caps; an OFAT benchmark must hold those axes fixed.
// C19 itself allows experiment requests without selection_provenance, so the physical
// compiler/renderer remains canonical without forging or weakening a C49 receipt.
...
for(const [index,candidate] of candidates.entries()){
  const candidateId=candidate.candidate_id,variant=candidate.template_variant;
  ...
  const payload={...book,cover_url:`./generated-c50-motion-ofat/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,c50_motion_ofat:{candidate_id:candidateId,motion_grammar:motionFamily,index}};
  ...
  selected.push(row);rows.push({index,candidate_id:candidateId,selection_id:selectionId,template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id,motion_grammar:motionFamily,payload_sha256:payloadSha256,axes:variant.axes});
}
...
const request={schema:'framewright-c19-campaign-request-v1',campaign_id:'c50-motion-ofat',compiler_policy_version:'c19-delivery-package-v1',runtime,delivery_profiles:deliveryProfiles,selected,reserves:[]};
...
fs.writeFileSync(path.join(outDir,'materialization-manifest.json'),JSON.stringify({schema:'c50-motion-ofat-materialization-v1',selected_count:rows.length,narrative_plan_id:plan.narrative_plan_id,template_sha256:templateSha256,cover_sha256:coverSha256,invariant_axes:invariantAxes,varied_axis:'motion_grammar',motion_families:motionFamilies,selection_provenance:null,rows},null,2)+'\n');
console.log(JSON.stringify({schema:'c50-motion-ofat-fixture-v1',selectedCount:rows.length,variedAxis:'motion_grammar',motionFamilies,invariantAxes,narrativePlanId:plan.narrative_plan_id,templateSha256,coverSha256,audioSpecId:audioSpec.audio_spec_id,selectionProvenance:null},null,2));
