...
const receipts=layoutFamilies.map(structural_layout=>composeExplicitTemplateVariant({selection:{...fixedAxes,structural_layout},context,registry}));
...
for(const [index,receipt] of receipts.entries()){
  const variant=receipt.template_variant,sceneProgram=deriveTemplateSceneProgram({composition_receipt:receipt,narrative,delivery,trusted_cover:trustedCover,registry}),selectionId=`i22-layout-${String(index+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-i22-layout/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,i22_experiment:{arm:'layout_ofat',index}};
...
const request={schema:'framewright-c19-campaign-request-v1',campaign_id:'i22-layout-ofat-source',compiler_policy_version:'c19-delivery-package-v1',runtime,delivery_profiles:[{id:'vertical-i22-layout-v1',width:1080,height:1920,fps:30,safe_area_profile:'c26-ui-safe-v1-2026-09-17:youtube_shorts',platform_ui_profile:'youtube_shorts',platform_ui_version:'c26-ui-safe-v1-2026-09-17'}],selected,reserves:[]};
fs.writeFileSync(path.join(outDir,'campaign.json'),JSON.stringify(request,null,2)+'\n');
fs.writeFileSync(path.join(outDir,'materialization-manifest.json'),JSON.stringify({schema:'i22-layout-ofat-materialization-v1',selected_count:rows.length,narrative_plan_id:plan.narrative_plan_id,book,plan,cover_name:coverName,cover_sha256:coverSha256,trusted_cover:trustedCover,template_sha256:templateSha256,fixed_axes:fixedAxes,varied_axis:'structural_layout',rows},null,2)+'\n');
console.log(JSON.stringify({schema:'i22-layout-ofat-fixture-v1',selectedCount:rows.length,layouts:layoutFamilies,fixedAxes,narrativePlanId:plan.narrative_plan_id,coverSha256,templateSha256},null,2));
