...
const fixtureDir=path.resolve(process.argv[2]||'.bench/i23/c27-fixtures');
const html=path.resolve(process.argv[3]||'examples/book-ad-systems/index-c45-template-variant.html');
const audioSource=path.resolve(process.argv[4]||'.bench/i23/source.wav');
const audioArtifact=path.resolve(process.argv[5]||'.bench/i23/canonical-audio.m4a');
const outDir=path.resolve(process.argv[6]||'.bench/i23/source-input');
...
for(const file of [html,audioSource,audioArtifact,path.join(fixtureDir,'manifest.json')])if(!fs.existsSync(file))throw new Error(`missing I23 input ${file}`);
const sourceManifest=JSON.parse(fs.readFileSync(path.join(fixtureDir,'manifest.json'),'utf8'));
const item=sourceManifest.items.find(x=>x.bookId==='winter-map'&&x.angleId==='map-premise'&&x.revealTiming==='early'&&Number(x.durationSeconds)===9)||sourceManifest.items.find(x=>Number(x.durationSeconds)===9&&x.revealTiming==='early');
if(!item)throw new Error('no canonical early-reveal C27 9s fixture');
...
if(plan.duration_seconds!==9||plan.cta_treatment!=='intent')throw new Error(`I23 expects canonical 9s intent CTA, got ${plan.duration_seconds}s/${plan.cta_treatment}`);
const cta=plan.roles.find(role=>role.role==='cta');if(!cta||cta.start_frame!==Math.round(plan.total_frames*.82))throw new Error(`I23 expects canonical C27 CTA boundary at 82%, got ${cta?.start_frame}`);
const beforeCta=plan.roles.find(role=>role.end_frame===cta.start_frame);if(!beforeCta||beforeCta.role==='book_reveal')throw new Error('I23 requires a no-cover semantic role immediately before CTA for sealed background calibration');
...
const runtimeAssetDir=path.join(path.dirname(html),'generated-i23-motion');fs.mkdirSync(runtimeAssetDir,{recursive:true});
...
const motionFamilies=loadMotionGrammarFamilyRegistry().families.map(f=>f.id).sort();if(motionFamilies.length!==6)throw new Error(`expected 6 C40 motion families, got ${motionFamilies.length}`);
...
const audioBytes=fs.readFileSync(audioArtifact),canonicalAudio={schema:'framewright-canonical-audio-v1',audio_spec_id:audioSpec.audio_spec_id,sha256:shaBytes(audioBytes),bytes:audioBytes.byteLength,media_type:'audio/mp4',storage_uri:`artifact://i23/${path.basename(audioArtifact)}`,provenance:{encoder:'ffmpeg-github-runner',policy:audioSpec.codec.policy}};assertCanonicalAudioArtifact(audioSpec,canonicalAudio,audioBytes);
fs.rmSync(outDir,{recursive:true,force:true});fs.mkdirSync(outDir,{recursive:true});
const selected=[],rows=[];
for(const [index,receipt] of receipts.entries()){
  const variant=receipt.template_variant,sceneProgram=deriveTemplateSceneProgram({composition_receipt:receipt,narrative,delivery,trusted_cover:trustedCover,registry}),selectionId=`i23-motion-${String(index+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-i23-motion/${coverName}`,delivery_profile:'vertical',narrative_plan:plan,template_scene_program:sceneProgram,i23_experiment:{arm:'motion_ofat',index}};
  ...
  const template={id:'book-ad-template-variant',version:'i23-motion-roundtrip-v1',sha256:templateSha256},hook=narrative.roles.find(r=>r.role==='hook')?.text||'';
  selected.push({selection_id:selectionId,selection_order:index,creative:{schema:'newboo-creative-spec-v1',book_id:book.book_id,payload_sha256:payloadSha256,template,visual_system:{id:variant.axes.visual_system,version:'v2'},structural_variant:variant.axes.structural_layout,hook:{source:'c27_narrative_plan',text:hook,source_ref:`${plan.narrative_plan_id}:hook`},motion:{profile:variant.axes.motion_grammar,version:'v2'},art_direction:{mode:'template-variant',algorithm:'i23-motion-ofat-v1',source_cover_sha256:coverSha256,palette:{background:book.background||'#eee8dc',surface:'#ffffff',ink:book.ink||'#111111',accent:book.accent||'#cc332d',secondary:'#315A7D'}},seed:plan.seed,assets:[{role:'cover',sha256:coverSha256,media_type:'image/png',uri:`asset://generated-i23-motion/${coverName}`}],template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id},timeline:{source:`c27:${plan.narrative_plan_id}`,policy_version:plan.policy_version,duration_ms:durationMs,frame_count:frameCount},requested_delivery_profile_ids:['vertical-i23-motion-v1'],render_assets:[{role:'scene_payload',sha256:payloadSha256,media_type:'application/json',uri:`execution://${selectionId}/payload`}],audio:{spec:audioSpec,artifact:canonicalAudio},execution:{html:rel(html),payload:rel(payloadFile),audio:rel(audioArtifact),template_id:template.id,template_version:template.version}});
  rows.push({index,selection_id:selectionId,motion_grammar:variant.axes.motion_grammar,template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id,payload_sha256:payloadSha256});
}
...
const request={schema:'framewright-c19-campaign-request-v1',campaign_id:'i23-motion-ofat-source',compiler_policy_version:'c19-delivery-package-v1',runtime,delivery_profiles:[{id:'vertical-i23-motion-v1',width:1080,height:1920,fps:30,safe_area_profile:'c26-ui-safe-v1-2026-09-17:youtube_shorts',platform_ui_profile:'youtube_shorts',platform_ui_version:'c26-ui-safe-v1-2026-09-17'}],selected,reserves:[]};
fs.writeFileSync(path.join(outDir,'campaign.json'),JSON.stringify(request,null,2)+'\n');
fs.writeFileSync(path.join(outDir,'materialization-manifest.json'),JSON.stringify({schema:'i23-motion-ofat-materialization-v1',selected_count:rows.length,narrative_plan_id:plan.narrative_plan_id,book,plan,cover_name:coverName,cover_sha256:coverSha256,trusted_cover:trustedCover,template_sha256:templateSha256,fixed_axes:fixedAxes,varied_axis:'motion_grammar',motion_families:motionFamilies,selection_provenance:null,rows},null,2)+'\n');
console.log(JSON.stringify({schema:'i23-motion-ofat-fixture-v1',selectedCount:rows.length,motionFamilies,fixedAxes,narrativePlanId:plan.narrative_plan_id,ctaStartFrame:cta.start_frame,coverSha256,templateSha256,selectionProvenance:null},null,2));
