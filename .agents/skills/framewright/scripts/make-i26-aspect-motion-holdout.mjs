...
const fixtureDir=path.resolve(process.argv[2]||'.bench/i26/c27-fixtures');
const html=path.resolve(process.argv[3]||'examples/book-ad-systems/index-c45-template-variant.html');
const audioSource=path.resolve(process.argv[4]||'.bench/i26/source.wav');
const audioArtifact=path.resolve(process.argv[5]||'.bench/i26/canonical-audio.m4a');
const outDir=path.resolve(process.argv[6]||'.bench/i26/source-input');
...
for(const file of [html,audioSource,audioArtifact,path.join(fixtureDir,'manifest.json')])if(!fs.existsSync(file))throw new Error(`missing I26 input ${file}`);
...
if(!item)throw new Error('I26 requires one early 9s C27 fixture');
...
if(plan.duration_seconds!==9||plan.cta_treatment!=='intent')throw new Error('I26 expects canonical 9s intent CTA');
...
const runtimeAssetDir=path.join(path.dirname(html),'generated-i26-aspect-motion');fs.rmSync(runtimeAssetDir,{recursive:true,force:true});fs.mkdirSync(runtimeAssetDir,{recursive:true});
...
const profiles=[
  {aspect:'vertical',profile:{id:'vertical-i26-motion-v1',width:1080,height:1920,fps:30,safe_area_profile:'c26-ui-safe-v1-2026-09-17:youtube_shorts',platform_ui_profile:'youtube_shorts',platform_ui_version:'c26-ui-safe-v1-2026-09-17'}},
  {aspect:'square',profile:{id:'square-i26-motion-v1',width:1080,height:1080,fps:30,safe_area_profile:'e17-square-v1'}},
  {aspect:'landscape',profile:{id:'landscape-i26-motion-v1',width:1920,height:1080,fps:30,safe_area_profile:'e17-landscape-v1'}},
];
const registry=buildCanonicalTemplateVariabilityRegistry(),motions=loadMotionGrammarFamilyRegistry().families.map(f=>f.id).sort();if(motions.length!==6)throw new Error(`I26 expected 6 C40 families, got ${motions.length}`);
...
const audioBytes=fs.readFileSync(audioArtifact),canonicalAudio={schema:'framewright-canonical-audio-v1',audio_spec_id:audioSpec.audio_spec_id,sha256:shaBytes(audioBytes),bytes:audioBytes.byteLength,media_type:'audio/mp4',storage_uri:`artifact://i26/${path.basename(audioArtifact)}`,provenance:{encoder:'ffmpeg-github-runner',policy:audioSpec.codec.policy}};assertCanonicalAudioArtifact(audioSpec,canonicalAudio,audioBytes);
fs.rmSync(outDir,{recursive:true,force:true});fs.mkdirSync(outDir,{recursive:true});
const selected=[],rows=[];let order=0;
for(const [aspectIndex,{aspect,profile}] of profiles.entries())for(const [motionIndex,motion_grammar] of motions.entries()){
  const delivery={aspect,width:profile.width,height:profile.height,fps:profile.fps,duration_seconds:9},context={aspect,duration_seconds:9,semantic_roles:narrative.roles.map(r=>r.role),available_asset_kinds:['cover']};
  const receipt=composeExplicitTemplateVariant({selection:{...fixedAxes,motion_grammar},context,registry}),variant=receipt.template_variant,sceneProgram=deriveTemplateSceneProgram({composition_receipt:receipt,narrative,delivery,trusted_cover:trustedCover,registry});
  const selectionId=`i26-cell-${String(aspectIndex+1).padStart(2,'0')}-${String(motionIndex+1).padStart(2,'0')}`;
  const payload={...book,cover_url:`./generated-i26-aspect-motion/${coverName}`,delivery_profile:aspect,narrative_plan:plan,template_scene_program:sceneProgram,i26_experiment:{arm:'aspect_motion_holdout',aspect_index:aspectIndex,motion_index:motionIndex}};
  ...
  const template={id:'book-ad-template-variant',version:'i26-aspect-motion-holdout-v1',sha256:templateSha256},hook=narrative.roles.find(r=>r.role==='hook')?.text||'';
  selected.push({selection_id:selectionId,selection_order:order++,creative:{schema:'newboo-creative-spec-v1',book_id:book.book_id,payload_sha256:payloadSha256,template,visual_system:{id:variant.axes.visual_system,version:'v2'},structural_variant:variant.axes.structural_layout,hook:{source:'c27_narrative_plan',text:hook,source_ref:`${plan.narrative_plan_id}:hook`},motion:{profile:variant.axes.motion_grammar,version:'v2'},art_direction:{mode:'template-variant',algorithm:'i26-aspect-motion-holdout-v1',source_cover_sha256:coverSha256,palette:{background:book.background||'#eee8dc',surface:'#ffffff',ink:book.ink||'#111111',accent:book.accent||'#cc332d',secondary:'#315A7D'}},seed:plan.seed,assets:[{role:'cover',sha256:coverSha256,media_type:'image/png',uri:`asset://generated-i26-aspect-motion/${coverName}`}],template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id},timeline:{source:`c27:${plan.narrative_plan_id}`,policy_version:plan.policy_version,duration_ms:durationMs,frame_count:plan.total_frames},requested_delivery_profile_ids:[profile.id],render_assets:[{role:'scene_payload',sha256:payloadSha256,media_type:'application/json',uri:`execution://${selectionId}/payload`}],audio:{spec:audioSpec,artifact:canonicalAudio},execution:{html:rel(html),payload:rel(payloadFile),audio:rel(audioArtifact),template_id:template.id,template_version:template.version}});
  rows.push({selection_id:selectionId,aspect_index:aspectIndex,motion_index:motionIndex,aspect,delivery_profile_id:profile.id,motion_grammar:variant.axes.motion_grammar,template_variant_id:variant.template_variant_id,scene_program_id:sceneProgram.scene_program_id,payload_sha256:payloadSha256});
}
...
const request={schema:'framewright-c19-campaign-request-v1',campaign_id:'i26-aspect-motion-holdout-source',compiler_policy_version:'c19-delivery-package-v1',runtime,delivery_profiles:profiles.map(x=>x.profile),selected,reserves:[]};
fs.writeFileSync(path.join(outDir,'campaign.json'),JSON.stringify(request,null,2)+'\n');
fs.writeFileSync(path.join(outDir,'materialization-manifest.json'),JSON.stringify({schema:'i26-aspect-motion-holdout-materialization-v1',selected_count:rows.length,aspect_count:profiles.length,motion_family_count:motions.length,book_id:book.book_id,narrative_plan_id:plan.narrative_plan_id,cover_sha256:coverSha256,template_sha256:templateSha256,fixed_axes:fixedAxes,varied_axes:['delivery.aspect','motion_grammar'],profiles:profiles.map(x=>({aspect:x.aspect,...x.profile})),motions,selection_provenance:null,rows},null,2)+'\n');
console.log(JSON.stringify({schema:'i26-aspect-motion-holdout-fixture-v1',selectedCount:rows.length,bookId:book.book_id,aspects:profiles.map(x=>x.aspect),motionFamilies:motions,fixedAxes,selectionProvenance:null},null,2));
