...
const reportPath=path.resolve(process.env.REPORT||'artifacts/e09/soak.json');
const width=+(process.env.WIDTH||1080), preset=process.env.PRESET||'veryfast', crf=+(process.env.CRF||22);
const soakVideos=Math.max(1,+(process.env.SOAK_VIDEOS||100));
...
function median(xs){const s=[...xs].sort((a,b)=>a-b);return s.length?s[Math.floor((s.length-1)/2)]:0;}
function slope(xs){if(xs.length<2)return 0;const mx=(xs.length-1)/2,my=xs.reduce((a,b)=>a+b,0)/xs.length;let n=0,d=0;for(let i=0;i<xs.length;i++){n+=(i-mx)*(xs[i]-my);d+=(i-mx)**2;}return d?n/d:0;}
...
async function renderOne(entry,seq){
  ...
  const seed=+(entry.seed??7),id=entry.id||`item-${seq}`,tmp=path.join(os.tmpdir(),`framewright-e09-${process.pid}.mp4`);
  ...
  const ff=spawn('ffmpeg',['-hide_banner','-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgba','-s',`${width}x${height}`,'-r',String(fps),'-i','-','-c:v','libx264','-preset',preset,'-crf',String(crf),'-maxrate','14M','-bufsize','28M','-pix_fmt','yuv420p','-movflags','+faststart',tmp],{stdio:['pipe','ignore','pipe']});
  let ffErr='',peakCombinedRss=0;ff.stderr.on('data',d=>ffErr+=d.toString());const t0=performance.now();
  for(let i=0;i<total;i++){
    const rgba=context.__renderRaw(i,width,seed),buf=Buffer.from(rgba.buffer,rgba.byteOffset,rgba.byteLength);
    if(!ff.stdin.write(buf))await new Promise(r=>ff.stdin.once('drain',r));
    if(i%30===0)peakCombinedRss=Math.max(peakCombinedRss,rssBytes(process.pid)+rssBytes(ff.pid));
  }
  ff.stdin.end();await new Promise((res,rej)=>{ff.on('error',rej);ff.on('close',code=>code===0?res():rej(new Error(`ffmpeg ${code}: ${ffErr}`)));});
  const totalMs=performance.now()-t0,warnings=uniqueWarnings(context.__layoutWarnings||[]),outputBytes=fs.statSync(tmp).size;fs.rmSync(tmp,{force:true});
  return{seq,id,seed,initMs:+initMs.toFixed(3),totalMs:+totalMs.toFixed(3),fps:+(total/(totalMs/1000)).toFixed(3),outputBytes,peakCombinedRssBytes:peakCombinedRss,nodeRssAfterBytes:rssBytes(process.pid),layoutWarnings:warnings};
}

const t0=performance.now(),results=[];let peakCombinedRss=0,warningGroups=0;
for(let i=0;i<soakVideos;i++){
  const r=await renderOne(manifest.items[i%manifest.items.length],i+1);results.push(r);peakCombinedRss=Math.max(peakCombinedRss,r.peakCombinedRssBytes);warningGroups+=r.layoutWarnings.length;
  if((i+1)%10===0||i===0)console.log(`${i+1}/${soakVideos} ${(r.totalMs/1000).toFixed(2)}s nodeRSS=${(r.nodeRssAfterBytes/1048576).toFixed(1)}MiB peak=${(peakCombinedRss/1048576).toFixed(1)}MiB`);
}
const totalMs=performance.now()-t0,times=results.map(r=>r.totalMs),rss=results.map(r=>r.nodeRssAfterBytes),q=Math.max(1,Math.floor(rss.length/4)),first=median(rss.slice(0,q)),last=median(rss.slice(-q)),cpu=os.cpus();
let canvasVersion=null;try{canvasVersion=JSON.parse(fs.readFileSync(path.resolve('node_modules/@napi-rs/canvas/package.json'),'utf8')).version;}catch{}
let ffmpegVersion=null;try{ffmpegVersion=execFileSync('ffmpeg',['-version'],{encoding:'utf8'}).split('\n')[0];}catch{}
const report={schema:'framewright-e09-soak-v1',renderer:'@napi-rs/canvas',canvasVersion,ffmpegVersion,font:'DejaVu Sans pinned',host:{platform:process.platform,arch:process.arch,node:process.version,cpus:cpu.length,cpuModel:cpu[0]?.model||null,totalMemoryBytes:os.totalmem()},width,height:Math.round(width*16/9),preset,crf,count:results.length,totalMs:+totalMs.toFixed(3),videosPerHour:+(results.length*3600000/totalMs).toFixed(2),perVideoMs:stats(times),peakCombinedRssBytes:peakCombinedRss,layoutWarningGroups:warningGroups,memory:{nodeRssMinBytes:Math.min(...rss),nodeRssMaxBytes:Math.max(...rss),firstQuartileMedianBytes:first,lastQuartileMedianBytes:last,quartileDeltaBytes:last-first,linearSlopeBytesPerVideo:+slope(rss).toFixed(3)},results};
fs.writeFileSync(reportPath,JSON.stringify(report,null,2));console.log(JSON.stringify({count:report.count,totalMin:+(report.totalMs/60000).toFixed(2),videosPerHour:report.videosPerHour,p50s:+(report.perVideoMs.p50/1000).toFixed(3),p95s:+(report.perVideoMs.p95/1000).toFixed(3),peakCombinedRssMiB:+(peakCombinedRss/1048576).toFixed(1),nodeRssFirstQMiB:+(first/1048576).toFixed(1),nodeRssLastQMiB:+(last/1048576).toFixed(1),rssSlopeKiBPerVideo:+(report.memory.linearSlopeBytesPerVideo/1024).toFixed(1),layoutWarningGroups:warningGroups,canvasVersion},null,2));
