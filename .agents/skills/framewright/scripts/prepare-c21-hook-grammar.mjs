...
const OPENING_GRAMMAR=String((window.FRAMEWRIGHT_PAYLOAD&&window.FRAMEWRIGHT_PAYLOAD.opening_grammar)||'question-led').toLowerCase();
const OPENING_GRAMMARS=new Set(['question-led','cover-led','title-led','fragment-reveal']);
const ACTIVE_OPENING_GRAMMAR=OPENING_GRAMMARS.has(OPENING_GRAMMAR)?OPENING_GRAMMAR:'question-led';
...
function __c21Fragment(text){
  const words=__c21Words(text),n=Math.max(3,Math.min(8,Math.ceil(words.length*.42)));
  return words.slice(0,n).join(' ')+(words.length>n?'…':'');
}
function __c21OpeningOverlay(g,S,style){
  if(ACTIVE_OPENING_GRAMMAR==='question-led'||S.t>.72)return;
