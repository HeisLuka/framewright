...
async function scenario(mode){const browser=await launch(), refs=await referenceHashes(browser), rows=[],failures=[];let peak=0;const c0=cpu(),t0=performance.now();const timer=setInterval(()=>peak=Math.max(peak,rss()),40);try{const workers=await Promise.all(Array.from({length:concurrency},async()=>mode==='navigate'?browser.newPage():null));await Promise.all(Array.from({length:concurrency},async(_,w)=>{for(let j=w;j<jobsN;j+=concurrency){const idx=j%entries.length,e=entries[idx],seed=e.seed+(j%5),start=performance.now();let p=workers[w];try{if(mode==='fresh')p=await browser.newPage();const loadMs=await navigate(p,idx,seed);const got=hashRows(await visualHashes(p));const parity=got.every((x,k)=>x.sha256===refs[idx][k].sha256);if(!parity)throw new Error(`visual parity failed ${e.bookId}`);const en=await encode(p,`${mode}-${j}`,e,seed);const muxMs=await finish(`${mode}-${j}`,en);rows.push({j,bookId:e.bookId,loadMs,encodeMs:en.encodeMs,muxMs,wallMs:performance.now()-start,parity});}catch(err){failures.push({j,bookId:e.bookId,error:String(err?.stack||err)})}finally{if(mode==='fresh'&&p)await p.close().catch(()=>{})}}}));for(const p of workers)if(p)await p.close().catch(()=>{});}finally{clearInterval(timer);await browser.close();}const wall=performance.now()-t0,cpuMs=(cpu()-c0)/1000;const vals=rows.map(x=>x.wallMs).sort((a,b)=>a-b),q=p=>vals[Math.min(vals.length-1,Math.floor((vals.length-1)*p))]||0;return{mode,jobs:rows.length,failures,scenarioWallMs:+wall.toFixed(2),videosPerHour:+(rows.length*3600000/wall).toFixed(2),p50JobMs:+q(.5).toFixed(2),p95JobMs:+q(.95).toFixed(2),cpuMs:+cpuMs.toFixed(2),cpuMsPerVideo:+(cpuMs/Math.max(1,rows.length)).toFixed(2),peakRssBytes:peak,allParity:rows.every(x=>x.parity),rows};}
try{console.log('R33 heterogeneous fixtures',entries.map(e=>`${e.bookId}:${e.style}`).join(', '));const fresh=await scenario('fresh');console.log('fresh',fresh);const navigateReuse=await scenario('navigate');console.log('navigate',navigateReuse);const gain=fresh.videosPerHour?navigateReuse.videosPerHour/fresh.videosPerHour-1:0;const report={schema:'framewright-r33-heterogeneous-warm-reset-v1',fixtures:entries.map(e=>({id:e.id,bookId:e.bookId,style:e.style,width:e.width,height:e.height,seed:e.seed})),config:{jobs:jobsN,concurrency,bitrate},results:{fresh,navigateReuse},decision:{navigateGainVsFresh:+gain.toFixed(4),correctnessPass:fresh.allParity&&navigateReuse.allParity&&!fresh.failures.length&&!navigateReuse.failures.length,deepSwapWorthTesting:gain>=.10}};await fsp.writeFile(reportPath,JSON.stringify(report,null,2)+'\n');const md=`# R33 heterogeneous warm reset\n\nFresh-page c2: **${fresh.videosPerHour} videos/h**; same page-object + full document navigation c2: **${navigateReuse.videosPerHour} videos/h** (${(gain*100).toFixed(1)}%).\n\nVisual parity against fresh per-book references: **${report.decision.correctnessPass?'PASS':'FAIL'}**.\n\nDecision: ${report.decision.deepSwapWorthTesting?'full in-document payload swap earned a deep pass':'do not add mutable in-document reset; full navigation keeps the safer contract unless another workload changes the result'}.\n`;await fsp.writeFile(path.join(outDir,'summary.md'),md);console.log(md);}finally{await new Promise(ok=>server.close(ok));}
