...
const output=path.resolve(process.argv[3]||'artifacts/r37/r37-worker.mjs');
let source=fs.readFileSync(input,'utf8');

function replaceOnce(needle,replacement,label){
  const first=source.indexOf(needle);
  if(first<0)throw new Error(`R37 transform marker missing: ${label}`);
  if(source.indexOf(needle,first+needle.length)>=0)throw new Error(`R37 transform marker not unique: ${label}`);
  source=source.slice(0,first)+replacement+source.slice(first+needle.length);
}

const warmupRe=/const warmupJobs = [^\n]+\n/;
const warmupMatch=source.match(warmupRe);
if(!warmupMatch)throw new Error('R37 warmup marker missing');
source=source.replace(warmupRe,warmupMatch[0]+String.raw`const scenario = String(process.env.RECYCLE_MODE || 'control').toLowerCase();
if (!['control','recycle'].includes(scenario)) throw new Error('RECYCLE_MODE must be control or recycle');
const batchSize = Math.max(concurrency, Math.trunc(Number(process.env.BATCH_SIZE || 6)));
...
if (jobsN < learnJobs + batchSize) throw new Error('R37 needs jobs beyond the learn window');
...
if(start<0||end<0||end<=start)throw new Error('R37 execution block markers missing');
const replacement=String.raw`  let workers = await Promise.all(Array.from({ length: concurrency }, () => browser.newPage()));
  ...
  async function runOne(page, worker, j) {
    ...
    const id = 'r37-' + scenario + '-' + j + '-' + crypto.createHash('sha1').update(entry.id).digest('hex').slice(0, 8);
  ...
  try {
    for (let batchStart = 0; batchStart < jobsN; batchStart += batchSize) {
      ...
      const sample = { batchStart, batchEnd, jobsCompleted: batchEnd, completionSeq, generation, rssBytes: rss, cgroupBytes: cgroup, elapsedScenarioMs: performance.now() - scenario0 };
      ...
      if (scenario !== 'recycle' || !recycleBaselineRss) continue;
...
replaceOnce("resetMode: 'full-document-navigation', recycleMode: 'observe-only'","resetMode: 'full-document-navigation', recycleMode: scenario, batchSize, learnJobs, thresholdMultiplier, streakRequired, idleSettleMs",'report config');
...
replaceOnce("schema: 'framewright-r34-mixed-catalog-soak-v1'","schema: 'framewright-r37-state-recycle-scenario-v1'",'report schema');
replaceOnce("# R34 mixed-catalog warm soak","# R37 ${scenario} mixed-catalog state-recycle scenario",'summary title');

fs.mkdirSync(path.dirname(output),{recursive:true});
fs.writeFileSync(output,source);
console.log(JSON.stringify({input,output,scenarioContract:{batchSize:6,learnJobs:36,thresholdMultiplier:1.25,streakRequired:3,idleSettleMs:100}},null,2));
