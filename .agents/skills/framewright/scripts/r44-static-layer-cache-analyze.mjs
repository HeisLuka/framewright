...
const [baseline,cached]=await Promise.all([basePath,cachedPath].map(async p=>JSON.parse(await fs.readFile(p,'utf8'))));
...
function summarize(r){
  ...
  return {
    jobs,
    serialMixVideosPerHour:jobs*3600000/wall,
    p95JobMs:q(all.map(x=>x.wallMs),.95),
    cpuMsPerVideo:rows.reduce((a,x)=>a+x.cpuMsPerVideo*x.jobs,0)/jobs,
    drawMsPerVideo:all.reduce((a,x)=>a+x.drawMs,0)/Math.max(1,all.length),
    peakRssBytes:Math.max(...rows.map(x=>x.peakRssBytes)),
    h264BytesPerVideo:all.reduce((a,x)=>a+x.h264Bytes,0)/Math.max(1,all.length),
    failures:rows.reduce((a,x)=>a+x.failures.length,0),
    parity:rows.every(x=>x.preflightParity)
  };
}
const b=summarize(baseline), c=summarize(cached);
const perProfile=Object.fromEntries(profiles.map(p=>{
  const x=pick(baseline,p),y=pick(cached,p);
  return [p,{
    baselineVph:x.videosPerHour,cachedVph:y.videosPerHour,throughputDelta:ratio(y.videosPerHour,x.videosPerHour),
    baselineP95:x.p95JobMs,cachedP95:y.p95JobMs,p95Delta:ratio(y.p95JobMs,x.p95JobMs),
    baselineRss:x.peakRssBytes,cachedRss:y.peakRssBytes,rssDelta:ratio(y.peakRssBytes,x.peakRssBytes),
    baselineCpu:x.cpuMsPerVideo,cachedCpu:y.cpuMsPerVideo,cpuDelta:ratio(y.cpuMsPerVideo,x.cpuMsPerVideo),
    parity:y.preflightParity,failures:y.failures.length
  }];
}));
const aggregate={
  throughputDelta:ratio(c.serialMixVideosPerHour,b.serialMixVideosPerHour),
  p95Delta:ratio(c.p95JobMs,b.p95JobMs),
  cpuDelta:ratio(c.cpuMsPerVideo,b.cpuMsPerVideo),
  drawDelta:ratio(c.drawMsPerVideo,b.drawMsPerVideo),
  rssDelta:ratio(c.peakRssBytes,b.peakRssBytes),
  bytesDelta:ratio(c.h264BytesPerVideo,b.h264BytesPerVideo)
};
const videosPerGiBBaseline=b.serialMixVideosPerHour/(b.peakRssBytes/2**30);
const videosPerGiBCached=c.serialMixVideosPerHour/(c.peakRssBytes/2**30);
aggregate.videosPerGiBDelta=ratio(videosPerGiBCached,videosPerGiBBaseline);
const correctness=c.failures===0&&c.parity&&profiles.every(p=>perProfile[p].parity&&!perProfile[p].failures);
