...
// Factory callers may pin exact codec/latency and expected geometry through env;
// the renderer must encode the scene canvas as-is and never crop/stretch it.
...
const expectedHeight=process.env.WEBCODECS_EXPECTED_HEIGHT==null?null:Number(process.env.WEBCODECS_EXPECTED_HEIGHT);
if(!fs.existsSync(html)){console.error(`no such HTML: ${html}`);process.exit(1);}
if(expectedHeight!=null&&(!Number.isInteger(expectedHeight)||expectedHeight<=0)){console.error(`invalid WEBCODECS_EXPECTED_HEIGHT=${process.env.WEBCODECS_EXPECTED_HEIGHT}`);process.exit(1);}
let payload=null;
...
const pageUrl=`http://127.0.0.1:${port}/${htmlName}?f=0&w=${encodeURIComponent(width)}&s=${encodeURIComponent(seed)}`;
...
const result=await page.evaluate(async({seed,width,bitrate,codecCandidates,latencyMode,expectedHeight})=>{
  ...
  const total=window.RISO.total,fps=window.RISO.fps||30;
  window.RISO.frame(0,width,seed);
  const actualWidth=Number(canvas.width),actualHeight=Number(canvas.height);
  if(actualWidth!==width)throw new Error(`runtime width mismatch: expected ${width}, got ${actualWidth}`);
  ...
  if(!selected)throw new Error(`no supported H.264 WebCodecs config at ${actualWidth}x${actualHeight}; requested=${codecCandidates.join(',')}`);
  ...
  try{
    for(let n=0;n<total;n++){
      ...
      if(canvas.width!==actualWidth||canvas.height!==actualHeight)throw new Error(`canvas geometry drift at frame ${n}: ${canvas.width}x${canvas.height} != ${actualWidth}x${actualHeight}`);
  ...
  return{base64,meta:{total,fps,width:actualWidth,height:actualHeight,codec:selected,encodedBytes:totalBytes,chunks:chunks.length,browserWallMs,renderMs,videoFrameMs,enqueueMs,base64Ms,maxQueue,decoderConfig,isSecureContext:self.isSecureContext}};
},{seed,width,bitrate,codecCandidates,latencyMode,expectedHeight});
...
const report={
  schema:'framewright-webcodecs-render-v5-delivery-geometry',createdAt:new Date().toISOString(),
  config:{html,payloadPath,seed,width,expectedHeight,bitrate,requestedCodec:requestedCodec||null,latencyMode},
