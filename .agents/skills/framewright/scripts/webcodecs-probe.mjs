...
// Writes a small JSON report; no video template is required.
...
await page.goto('data:text/html,<canvas id="c" width="320" height="568"></canvas>',{waitUntil:'load'});
const probe=await page.evaluate(async()=>{
  const result={videoEncoder:typeof VideoEncoder!=='undefined',videoFrame:typeof VideoFrame!=='undefined',configs:[],smoke:null};
  ...
  for(const c of candidates){
    try{const s=await VideoEncoder.isConfigSupported(c.config);result.configs.push({name:c.name,supported:Boolean(s.supported),config:s.config||c.config});}
  ...
  const enc=new VideoEncoder({
    output(chunk,meta){chunks.push({type:chunk.type,timestamp:chunk.timestamp,duration:chunk.duration,bytes:chunk.byteLength});if(meta?.decoderConfig&&!decoderConfig)decoderConfig={codec:meta.decoderConfig.codec,descriptionBytes:meta.decoderConfig.description?.byteLength||0};},
  ...
  result.smoke={codec:selected.name,config,wallMs:performance.now()-started,chunks:chunks.length,totalBytes:chunks.reduce((a,x)=>a+x.bytes,0),keyChunks:chunks.filter(x=>x.type==='key').length,firstChunks:chunks.slice(0,5),decoderConfig,error};
  return result;
});
await browser.close();
const report={schema:'framewright-webcodecs-probe-v1',createdAt:new Date().toISOString(),host:{platform:process.platform,arch:process.arch,node:process.version,cpus:os.cpus().length,cpuModel:os.cpus()[0]?.model||null},chromeLaunchMs:+launchMs.toFixed(3),probe};
fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(report,null,2));
console.log(JSON.stringify(report,null,2));
if(!probe.videoEncoder||!probe.configs.some(x=>x.supported))process.exitCode=2;
